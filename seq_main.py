...
def dataLoader(path, batch_size, dims, model1, model2, reverse=True):
...
#data_path = 'tgl-eng/all-arranged.txt'
#valid_path = 'tgl-eng/valid-arranged.txt'
data_path = 'tgl-eng/train.txt'
...
valid_input_texts, valid_target_texts = input2target(valid_path, sos, eos)
#test_input_texts, test_target_texts = input2target(test_path, sos, eos)
...
model.fit_generator(dataLoader(path=data_path,
                ...
                model1=filModel, model2=engModel, reverse=True),
...
decoder_model.save('s2s-decoder.h5')
encoder_model.save('s2s-encoder.h5')
...
indexes = np.random.randint(int(len(valid_input_texts)*3/4), len(valid_input_texts), size=100)
reversed_input = True
ctr = 0
